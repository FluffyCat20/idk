...
void data_2d_writer::output_for_current_time(double time) {
  ...
  for (size_t i = 0; i < par.size_x; ++i) {
    for (size_t j = 0; j < par.size_y; ++j) {
      const data_node_2d& pt = grid.get_const_mesh()[j][i];
...
void data_2d_writer::output_on_symmetry_axis_for_current_time(
    double time) {
...
void data_2d_writer::output_pressure_sensors_on_wall(
  ...
  auto pressure_sensors_on_wall =
    grid.get_const_pressure_sensors_on_wall();
